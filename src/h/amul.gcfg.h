...
static inline _RANK_STRUCT &
GetRank(int n) noexcept
{
    return g_game.m_ranks[n];
}
static inline _ROOM_STRUCT &
GetRoom(roomid_t n) noexcept
{
    return g_game.m_rooms[n];
}
static inline _ADJECTIVE &
GetAdj(adjid_t n) noexcept
{
    return g_game.m_adjectives[n];
}
static inline _OBJ_STRUCT &
