...
{	register int i;
	...
	if(argc>1)
	{
		switch(toupper(*(argv[1]+1)))
		{
			...
			case 'S':		/* Serial Device */
				switchS((argc>2)?argv[2]:NULL,(argc>3)?argv[3]:NULL,(argc>4)?argv[4]:"0",(argc>5)?argv[5]:"Y"); break;
			case 'N':		/* NTSC Screen */
				switchC(argc,'N'); break;
	...
	if((port=FindPort(mannam))==NULL) { tx("Manager not running!\n"); quit(); }
	if((reply=CreatePort(0L,0L))==NULL) memfail("user port");
	if((repbk=CreatePort(0L,0L))==NULL) memfail("comms reply");
	if((amanrep=CreatePort(0L,0L))==NULL) memfail("aman port");
	...
	*amanp = *amul;
	...
	if(Af >= MAXU) Special_Proc();

	/* Clear room flags, and send scenario */
	rset=(1 << Af); rclr=-1-rset;
	for(i=0; i < rooms; i++) *(rctab+i)=(*(rctab+i) & rclr);

	do	/* Print the title */
	{
		i=fread(block,1,900,ifp); block[i]=0; tx(block);
	} while(i==900);
	fclose(ifp); ifp=NULL;
	...
	do
	{
		...
		if(stricmp(input,"***debug")==NULL) { debug=debug ^ 1; continue; }
		if(input[0]==0) continue;
gloop:		failed=NO; forced=0; died=0; exeunt=0;
		if(grab()==-1) break;
		iocheck();
		if(forced!=0 && died==0 && exeunt==0) goto gloop;
	} while(exeunt==0 && died==0);

quitgame:		/* Quite the game, tidily. */
	if(died==0) action(acp(EXITED),AGLOBAL);
	       else action(acp(HEDIED),AGLOBAL);
...
{	register int own,orm;
	...
	if(STATE->flags & SF_LIT)
	{
		...
		{
			if(orm == (lstat+own)->room) return;
		}
		else
		{	/*== If I got it from someone, and he is near me... */
			if((lstat+own)->room == (lstat+to)->room) return;
