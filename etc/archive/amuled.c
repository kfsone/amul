...
main(int argc, char *argv[])
{
    ...
    if (argc != 2)
        error("%s Usage error, try: %s <game path>!\n", argv[0]);
    p = argv[1];
    strcpy(dir, p);
    ...
    if ((sc = OpenScreen(&NewScreenStructure)) == NULL)
        error("%s Can't open %s\n", "screen");
    vp = sc->ViewPort;
    LoadRGB4(&vp, (UWORD *)&Palette, PaletteColorCount);
    if ((mywin = DupeNewWindow(&AENewWindowStructure1)) == NULL)
        error("%s Can't open %s\n", "window");
    if ((myreq = DupeNewWindow(&RENewWindowStructure2)) == NULL)
        error("%s Can't open %s\n", "window");
    if ((mymen = DupeMenu(&AEMenuList1)) == NULL)
        error("%s Can't open %s\n", "window");
    mywin->Screen = sc;
    myreq->Screen = sc;
    if ((win = OpenWindow(mywin)) == NULL)
        error("%s Can't open %s\n", "window");
...
quit()
{
    ...
    if (mymen)
        DeleteMenu(mymen);
    if (myreq)
        DeleteNewWindow(myreq);
    myreq = NULL;
    if (mywin)
        DeleteNewWindow(mywin);
    mywin = NULL;
    if (ITBase)
        CloseLibrary(ITBase);
    ITBase = NULL;
...
gadg_proc()
{
    ...
    case gUSERNO:
        SetNo();
        Update();
        ActivateGadget(GetGadget(win, gNAME), win, NULL);
        break;
    ...
    if (gadg->UserData != NULL)
        ActivateGadget((struct Gadget *)gadg->UserData, win, NULL);
    else
        ActivateGadget(GetGadget(win, gNAME), win, NULL);
}
...
moLast()
{
    ...
    fopenr(plyrfn);
    fseek(ifp, 0, 2);
    uno = ftell(ifp) / sizeof(me);
    User();
}

moSave()
{
    if (request("=== Save User? ===") == FALSE)
        return;
    fopenr(plyrfn);
    fseek(ifp, (uno - 1) * sizeof(me), 0);
    fwrite(me.name, sizeof(me), 1, ifp);
    closei();
}

moDelete()
{
    if (request("== Delete User? ==") == FALSE)
        return;
    fopenr(plyrfn);
    fseek(ifp, (uno - 1) * sizeof(me), 0);
    fwrite(blank.name, sizeof(me), 1, ifp);
    closei();
    userpack();
    User();
}
...
userpack()
{
    ...
    sprintf(block, "%s%s.new", dir, plyrfn);
    unlink(block); /* Incase */
    if ((ofp = fopen(block, "ab+")) == NULL) {
        DisplayBeep(NULL);
        return;
    }
    ...
    rewind(ifp);
    do {
        fread(me.name, sizeof(me), 1, ifp);
        if (me.name[0] == 0 || feof(ifp))
            continue;
...
request(char *title)
{
    ...
    ClearIntuiMessages(win);
    myreq->Title = title;
    if ((req = FlashyOpenWindow(myreq)) == NULL) {
        DisplayBeep(sc);
        return FALSE;
    }
    do {
        imsg = GetIntuiMessage(req);
    } while (imsg->Class != GADGETUP && imsg->Class != GADGETDOWN);
    gad = (struct Gadget *)imsg->IAddress;
    ReplyMsg(imsg);
    ClearIntuiMessages(req);
    FlashyCloseWindow(req);
    return (gad->GadgetID == 100) ? TRUE : FALSE;
}

SetNo() { SetStringInfoNumber(win, gUSERNO, ftell(ifp) / sizeof(me), 10); }

Update() { RefreshGadgets(GetGadget(win, gNAME), win, NULL); }
...
loop:
    fseek(ifp, (uno - 1) * sizeof(me), 0);
    if (feof(ifp) || fread(me.name, sizeof(me), 1, ifp) != 1 && uno != 1) {
        dirn = -1;
        uno--;
        goto loop;
    }
    for (i = strlen(me.name); i < NAMEL + 1; i++)
        me.name[i] = 0;
    for (i = strlen(me.passwd); i < 23; i++)
        me.passwd[i] = 0;
    if (me.name[0] == 0) {
        uno += dirn;
        goto loop;
...
{
    fprintf(stderr, s, "AMUL-Ed error: ", s1, s2, s3, s4, s5);
    quit();
}
