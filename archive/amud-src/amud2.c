...
{	register int i;
	...
	if(!(me2->flags & PFASLEEP)) look((rmtab+myROOM)->id, me->rdmode);
...
{	register int o,or; register char *p;
	...
	*(p=block)=0;
	...
	tx(block); if(needcr==YES) txc('\n');
	if(i && p==block) sys(NOWTSPECIAL);
}
...
ahit(int f)		// Take a single blow
{	struct _RANK_STRUCT *trank;

	int	str,mdex,msta,ydex,ysta;
	...
	trank=rktab+(ranks-1);
	msta=PCTG(me->stamina,trank->stamina);	ysta=PCTG(you->stamina,trank->stamina);
...
ascore(int type) {
	calcdext();
	if(type == TYPEV) {
		txn("Player: @me the @mr (@gn).\nScore: @sc points. Scored this game: @sg points. Played: %6ld\n",me->plays);
		...
		sprintf(block,"Score: @sc. Strength: @sr/%ld. Stamina: @st/%ld. Dexterity: %ld/%ld. Magic: @mg/%ld\n",
