//
// AMUD/frame/FileBits.C	Miscellaneous functions
//
...
givebackmemory() {
	...
	if(amanrep)	DeleteAPort(amanrep,amud);
...
quit() {
	...
	if(link>0 && amud->from>-1) SendIt(MDISCNCT,0,me->name);

	if(ifp)	fclose(ifp);	// Close the various files we have open
	if(afp)	fclose(afp);
...
SendIt(int t,int d,char *p) {
	if(!link) return;
	AMt=t; AMd=d; AMp=p;
	PutMsg(port, (struct Message *) amanp); WaitPort(amanrep); GetMsg(amanrep);
