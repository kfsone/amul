//
// AMUD/frame/IOBits.C		Various IO Functions
//

// Some portions of this code is AmigaDOS specific, i.e. message port handling.
// Message Ports are the Amiga's means of inter-process communications, I would
// assume they are somewhat similar to "sockets"

iocheck() {
	int needc;
	needc=needcr;
	REALiocheck(); addcr=NO;
	if(!died && !exeunt && needc==YES && needcr==NO) ARefresh();
}
...
endlok:	me2->IOlock=-1;
	if(i==1) action(acp(WOKEN),ACANSEE);
...
lockusr(int u){
	...
	do{
		...
		if(Ad!=u && !ip && Af<MAXU) { iocheck(); Ad=-1; }
	} while(Ad!=u);
	At=t; Ad=d; Ap=(char *)p;
}
