//
// AMUD/com/Trav_Proc.C		Processor for Travel.Txt
//
...
vbloop:		p=skipline(s=p);
		if(!*s) {	// Only complain if not a death room
...
vbproc:		nvbs=0; tt.pptr=(long *)-1;	// Process Verb List
		l=(long *)temp;
		do {				// Break list down to verb no.s
