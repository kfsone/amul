//
// AMUD/com/Lang_Proc.C		Processor for Lang.Txt and variable handler
//
...
lang_proc() {
	...
	fclose(fopenw(lang1fn)); fopena(lang1fn); ofp1=afp; afp=NULL;
	ofp1=fopenw(lang2fn); ofp2=fopenw(lang3fn); ofp3=fopenw(lang4fn);
...
synloop:	s=skipspc(skiplead("syntax=",s)); setslots(WNONE);
		verb.ents++; s=skipspc(skiplead("verb",s));
