...
static const char DEFAULT_SYS_TEXT[] = 
";\n"
"; Default %s.smg\n"
"; Generated by %s\n"
";\n"
	...
	*rank.male = *rank.female = 0;
	...
	if (!rank_item("score", s, rank.score)
		...
		|| !rank_item("max. objects carried", s,
					  rank.numobj) || !rank_item("player kill value", s, rank.minpksl) || !rank_item("task number", s, rank.tasks))
		return;
...
checksys(void)
{
	...
	warne("Creating DEFAULT %s.smg file.\n", g_block);

	ofp1 = fopen(g_block, "wb") ;
	if ( !ofp1 )
		quit("Unable to create file %s.\n", g_block);
	fprintf(ofp1, DEFAULT_SYS_TEXT, txtfile[TF_SYSTEM], vername, sysopt[SO_NAME], sysopt[SO_SESH], sysopt[SO_PORT], sysopt[SO_MSGO], sysopt[SO_RSCL], sysopt[SO_TSCL], sysopt[SO_SEE1], sysopt[SO_SEE2], sysopt[SO_NOISE], sysopt[SO_NOISE]);
