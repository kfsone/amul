...
chkp(char *p, arg_t type, int c, int z)
{
	...
	if (!*p)
	{
		error("%s: Missing paramters (%s '%s')\n",
			  ...
			  (z == 1) ? "condition" : "action", (z == 1) ? cond[c].name : action[c].name);
		return NULL;
	}
	...
	{							/* Processing lang tab? */
		...
		{						/* If it was an actual, but wrong type */
			error("%s: Invalid variable, '%s', after %s '%s'\n",
				  verb.id, p2, (z == 1) ? "condition" : "action", (z == 1) ? cond[c].name : action[c].name);
			return NULL;
		}
	}
		...
		default:
			...
			if (!(proc == 1 && type >= 0 && type <= 10))
			{
				...
				error("> Internal error, bad PTYPE (val: %d, item: %s)!\n",
					  word((proc == 1) ? verb.id : bobs[cur_room]->id), type, p2);
				return NULL;
			}
		}
