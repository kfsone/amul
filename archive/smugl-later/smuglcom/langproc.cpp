	...
	do
	{
	  ...
	  cmdloop:
		...
		else
		{
			...
			if (!*s)
			{
				...
				{
					sprintf(g_block, "Missing action after condition '%s'", cond[vt.condition].name);
					vbprob(g_block, ls);
					goto commands;
				}
				vt.condition = CALWAYS;
	  ...
	  writecna:
		if (vt_alloc % GROW_SIZE == 0)
		{
			...
			vttab = (struct VBTAB *) grow(vttab, (size_t) (new_alloc * sizeof(vt)), "Resizing 'vt' table");
			vtp = vttab + vt_alloc;
		}
		*(vtp++) = vt;
		vt_alloc++;
	  ...
	  writeslot:
		if (st_alloc % GROW_SIZE == 0)
		{
			...
			sttab = (struct SLOTTAB *) grow(sttab, new_alloc * sizeof(vbslot), "Resizing 'vbslot' table");
			stp = sttab + st_alloc;
		}
		*(stp++) = vbslot;
		st_alloc++;
...
{								/* Determine the actual value of a variable */
	...
	for (i = 0; i < NACTUALS; i++)
	{
		...
		case INOUN1:			/* noun 1 */
			...
			return -1;
		case IADJ2:
			return (vbslot.adj[1] == n || n == PREAL) ? actual[i].value : -1;
		case INOUN2:
			return (vbslot.wtype[1] == n || n == PREAL) ? actual[i].value : -1;
		default:
			return -1;
		}
	}
