...
read_in_advfn(void)
{
	...
	if ( fread(data->logfile, ADNAMEL + 1, 1, fp) < ADNAMEL + 1 )
		throw Smugl::FPReadError(datafile(advfn), errno, fp) ;
	if ( fscanf(fp, "%ld %ld %d %d %d %d %d %d\n",
		   &data->compiled, &data->time, &data->see_invis,
		   &data->all_see_invis, &data->minsgo, &data->rscale, &data->tscale, &data->port) < 8 )
		throw Smugl::FPReadError(datafile(advfn), errno, fp) ;
...
void
load_database(void* const membase)	// Load all the files in the database
{
	...
	if (g_debug > 1)
	{
		...
		for (i = 0; i < data->rooms; i++)
		{
			...
			for (conChild = room->conTent; conChild >= 0 && conChild <= ncontainers; conChild = containers[conChild].conNext)
			{
