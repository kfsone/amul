...
void
APIs::Logging::Log::vWrite(const char* const srcFilename_, const unsigned int srcLineNo_, const APIs::Logging::Level level_, const char* const format_, va_list& args)
{
	...
	fprintf(stderr, "%c|%s|%s:%u|", levels[level_], timeStr, srcFilename_, (unsigned int)srcLineNo_) ;
	vfprintf(stderr, format_, args) ;

	if ( level_ >= APIs::Logging::Fatal )
		abort() ;
	if ( level_ >= APIs::Logging::Terminal )
		exit(level_) ;
}
...
fetch_input(char *to, int length)
{
	...
	*p = 0;
	...
	do
	{
		...
		rd_fds = fds;
		while ((ret = select(max + 1, &rd_fds, NULL, NULL, NULL)) == -1 && (errno == EINTR || errno == EAGAIN)) ;
		if (ret == -1)
		{
			tx(INTERNAL_ERROR);
			sysLog.Perror(_FLT, "fetch_input::select()");
			/*ABORT*/
		}
