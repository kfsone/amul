...
const char *
message(msgno_t id)				// Convert umsg id into a string pointer
{
...
const char *
rightstr(const char *s, int len)	// Return EOS - len characters
{
	len = strlen(s) - len;
	if (len <= 0)
		return s;
...
esc(const char *code, char *to)
{
	...
	case 'f':					// @f codes are for <friend> - person you are helping
		//            if(c=='r' && me->helping!=-1) { strcpy(s,(usr+me->helping)->name); return 1; }
		//            if(c=='m' && me->followed!=-1) { strcpy(s,(usr+me->followed)->name); return 1; }
	...
	case 'g':					// Gender words
		...
		case 'n':				// @gn = gender name
			if ( me->sex ) return strcopy(to, "female") ;
			else return strcopy(to, "male") ;
		case 'e':				// @ge = he/she
			if ( me->sex ) return strcopy(to, "she") ;
			else return strcopy(to, "he") ;
		case 'o':				// @go = gender for owner, i.e. his/her
			if ( me->sex ) return strcopy(to, "her") ;
			else return strcopy(to, "his") ;
		case 'h':				// @gh = him/her
			if ( me->sex ) return strcopy(to, "her") ;
			else return strcopy(to, "him") ;
	...
	case 'm':
		...
		case 'e':				// @me = my name
			...
			//          case 'f': if(me->following==-1) strcpy(s,"no-one"); else strcpy(s,(usr+me->following)->name); return 1;
	...
	case 'o':					// @o1 / @o2 = weapons
		...
		//if(c=='2' && (lstat+(me->fighting))->wield!=-1) { strcpy(s,(obtab+((lstat+(me->fighting))->wield))->id); return 1; }
	...
	case 'p':
		...
		if (second == 'o')		// @po = players online
		{
			...
			if (online == 1)
				...
				return (to + sprintf(to, "%s %d other %s",
									 (online > 2) ? "are" : "is", online - 1, (online > 2) ? "players" : "player"));
		}
		break;
