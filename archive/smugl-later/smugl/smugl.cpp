...
main(int argc, char* argv[])
{
	...
	srand((unsigned int)time(NULL));	///TODO: Not good enough; two people can log in the same second.
	...
	if (argc >= 1)				// We have some arguments
	{
		...
		for (; arg < argc; arg++)
		{
			...
			else
			{
				if (arg + 1 < argc)
				{
					sysLog.Write(_FLE, "Don't understand arguments at: %s %s ...", argv[arg], argv[arg + 1]);
					usage(1);
					/* NOTREACHED */
				}
