...
special_word(char* token)
{
	if (strcmp(token, "me") == 0 || strcmp(token, "myself") == 0 || strcmp(token, "self") == 0)
		return me->id;
	if (last_him && (strcmp(token, "he") == 0 || strcmp(token, "him") == 0 || strcmp(token, "his") == 0))
		return last_him->id;
	if (last_her && (strcmp(token, "she") == 0 || strcmp(token, "her") == 0 || strcmp(token, "hers") == 0))
		return last_her->id;
	return vocUNKNOWN;
}
...
slot_process(SLOTTAB* slot)
{
	...
	{
		if (g_debug)
			txprintf("Entry#%d: cond=%s%d, action=%d\n", ent, (vt->not_condition) ? "!" : "", vt->condition, vt->action);
			...
			for (i = 0; i < maybe.Vb->ents; i++)
			{
				...
				if (g_debug)
					txprintf("Slot %d has %ld entries for (%s:%ld,%s:%ld)\n",
							 i, slot->ents, syntax[slot->wtype[0] + 1], slot->slot[0], syntax[slot->wtype[1] + 1], slot->slot[1]);
