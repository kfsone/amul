...
login(void)						// Log a player in
{
	...
	{
		sysLog.Write(_FLD, "%s has %ld points for rank %ld but not %ld",
	...
	if (start_room == NULL)
	{
		...
		if (!start_room && first)
			start_room = first;
		else if (!start_room && !(start_room = RoomIdx::locate("start")))
		{						// Last ditch effort
...
static inline int
getname(void)					// Get the player's name
{
	char* p = new_name;
...
#ifdef NEVER
int
getpasswd(void)					// Existing user - verify password
{
