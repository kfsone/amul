...
//    class Player *guest;
	...
	if (l_descrip != -1 && me->rdmode != RDBF && !(me->rdmode == RDRC && (visitor_bf & me->bitmask)))
	{
	...
	{							// Can't see objects in hide-aways
		...
		for (int entry = 0; entry < contents && conChild >= 0; entry++)
		{
			...
			if (!(child->type == PNOUN && (std_flags & bob_HIDEOBJ))
				&& !(child->type == PPLAYER && (std_flags & bob_HIDECRE)) && !(child->type == PMOBILE && (std_flags & bob_HIDECRE)))
			{
				if (child->describe())
					finish = '\n';
			}
...
{								// Called with a verb pointer
	...
	for (ttp = Tabptr(), i = 0; i < ttlines; i++, ttp++)
	{
		if (ttp->verb == vb->id)
		{
			...
			{
				tx("> There are instructions to exit this room, " "but I can't handle them yet.\n");
				return true;
			}
...
Room::exits(void)
{
	...
	for (int i = 0; i < cur_loc->ttlines; i++, ttp++)
	{
		txprintf("  %d:[%s %c%s %s %p]\n",
				 ...
				 (ttp->action_type != ACT_DO) ? word((data->roombase + ttp->action)->id) : action[ttp->action].name, ttp->pptr);
	}
}
