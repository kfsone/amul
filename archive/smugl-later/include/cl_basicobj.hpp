...
#define WRITE(field) \
    if ( fwrite((void *)(&field), sizeof(field), 1, file) < sizeof(field) ) throw Smugl::FPWriteError("basicobj file", errno, file)

#define READ(field) \
	if ( fread((void *)(&field), sizeof(field), 1, file) < sizeof(field) ) throw Smugl::FPReadError("basicobj file", errno, file)
