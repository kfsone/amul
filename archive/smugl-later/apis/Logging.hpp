...
/* Defines a logging interface, to be implemented in a platform-specific fashion, but provide a platform-agnostic interface. */
...
#define _FLW	__FILE__, __LINE__, APIs::Logging::Warning		// Warning that is not guaranteed to be significant.
#define _FLE	__FILE__, __LINE__, APIs::Logging::Error		// Bad behavior that the code is dealing with.
#define _FLT	__FILE__, __LINE__, APIs::Logging::Terminal		// Bad behavior that caused the code to exit.
#define _FLF	__FILE__, __LINE__, APIs::Logging::Fatal		// Internal bad behavior that is causing a core dump.
...
namespace APIs
{
	...
	public:
		//! Main constructor.
		//! @param[in]	name_			How the log entries will be identified either as a filename or a logging level, or possible both.
	...
	public:
		...
		void Write(const char* const srcFilename_, const unsigned int srcLineNo_, const APIs::Logging::Level level_, const char* const format_, ...)
		{
		...
		//! Write something to the log file, followed by the description of the last system error (perror).
		//! @param[in]	srcFileName_	Origin of the error, usually __FILE__
		//! @param[in]	srcLineNo_		Origin line within srcFileName_ of the error, i.e. __LINE__
		//! @param[in]	level_			Logging::Level at which to record the message
		//! @param[in]	prefix_			Prefix message.
		void Perror(const char* const srcFilename_, const unsigned int srcLineNo_, const APIs::Logging::Level level_, const char* const prefix_, ...)
		{
	...
	public:
		...
		//! @param[in]	level_			Logging::Level at which to record the message
		//! @param[in]	format_			"sprintf" style format mask
		void operator() (const char* const srcFilename_, const unsigned int srcLineNo_, const APIs::Logging::Level level_, const char* const format_, ...)
		{
			if ( WillLog(level_) == false )
				return ;
			va_list args ;
	...
	private:
		...
		void vWrite(const char* const srcFilename_, const unsigned int srcLineNo_, const APIs::Logging::Level level_, const char* const format_, va_list& args) ;
