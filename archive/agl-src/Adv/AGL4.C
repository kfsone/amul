...
//
//	File: AGL4.C		AGL System - Subset (4 of 4)
//
//	LMod: oliver 19/06/93	Support for mobile->verb (-2=new, -1=none)
...
Mob_Act() {
	register int v,i,x,l; int trys;

	if(mobile->verb==-1) return;	// No verb
	if(mobile->verb==-2) {		// Mobile hasn't acted before
		sprintf(block,"!%s",mobile->id);
		if((v=isverb(block))<NULL) { mobile->verb=-1; return; }
	} else v=mobile->verb;
	tt.verb=-1; inoun1=me2->rec; wtype[2]=WNOUN; trys=3;
loop:	if(--trys<0) return;
	i=Random((vbptr->ptr)->ents-1); donet=i; l=-1; ml=(vbptr->ptr)->ents; overb=iverb=v;
