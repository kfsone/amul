...
Player::name(void)
    ...
    if (_name[0])               // Does the player have a name?
        return _name;
    else return "(somebody)";   // Should probably trap this ;-)
    }
...
Player *
PlayerIdx::locate(long id)      // Locate player by ID number
    {
    int i;
...
Player *
PlayerIdx::locate(char *s)      // Locate player by name
    {
