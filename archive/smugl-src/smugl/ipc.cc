...
sem_lock(int semnum)
    ...
    if (data && semnum >= 0)    // No point if data isn't assigned
        ...
        if (!sems[semnum])      // Don't actually perform multiple locks
            ...
            while (semop(data->semid, &sop, 1) == -1 &&
...
setup_socket(short port, long addr, SOCK *sock, int listen_to)
    ...
    if (ret == -1)
        {
        if (errno == EADDRINUSE)
            error(LOG_ERR, "Port %s:%d already in use, unable to bind",
                  inet_ntoa(sock->sin_addr), port);
        else
            error(LOG_ERR, "bind(%s:%d): %s",
                  inet_ntoa(sock->sin_addr), port, strerror(errno));
        exit(1);
        }
...
init_sockets(void)
    ...
    command_sock_fd = setup_socket(data->port + 1, COMMAND_ADDR,
                                   &command_sock, 1);
    }
...
accept_connection(void)
    ...
    while ((conn_sock_fd = accept(listen_sock_fd,
...
accept_command(void)
    ...
    bzero(&conn_sock, addrlen);
    conn_sock_fd = accept(command_sock_fd,
                          (struct sockaddr *)&conn_sock, &addrlen);
    if (conn_sock_fd == -1)
        {
