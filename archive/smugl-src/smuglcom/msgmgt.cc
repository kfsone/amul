...
add_msg(const char *id)
    ...
    if (!msgfp)
        {                       /* Haven't opened message file yet */
        msgfp = fopen(datafile(umsgfn), "wb");
        if (msgfp == NULL)
            Err("writ", datafile(umsgfn));
        }

    if (i_alloc % GROW_SIZE == 0)
        {                       /* No indexes availble, need to grow */
        new_alloc = (i_alloc + GROW_SIZE) * sizeof(*msgitab);
        msgitab = (long *)grow(msgitab, new_alloc,
                               "Reorganising Msg Index Table");
    ...
    *msgip = ftell(msgfp);
    ...
    if (id && *id)
        ...
        if (id_alloc % GROW_SIZE == 0)
            ...
            msgidtab = (MSGID *)grow(msgidtab, new_alloc,
                                     "Reorganising MsgID Table");
