...
#define DEFAULT_STD \
    (bob_INPLAY | bob_SCENERY | bob_LIGHT | bob_SHINES | bob_LIT)
// Specify what flags aren't allowed for a room using handle_std_flags
#define FILTER_STD \
    (bob_INPLAY | bob_SCENERY | bob_COUNTER | bob_FLAMABLE | bob_SCALED)
            ...
            if ((flag = is_room_flag(ptr)) != -1)
              ...
              case rp_dmove:    // Inventory moves elsewhere if you die
                    ...
                    if (dmove == -1) // Don't know this room yet
                        {
                        dmv = (Dmove *)grow(NULL, sizeof(struct Dmove),
                                            "Tracking DMOVEs");
                        dmv->room = rmp->bob;
                        dmv->to = strdup(ptr);
            ...
            while (fgets(bufmem + mem_off, rdalloc - mem_off, ifp))
                ...
                while ((q = strrchr(base, '\n')) == NULL)
                    ...
                    if ((size_t)(mem_off + ROOMDSC_GROW_RATE) >= rdalloc)
                        {
                        rdalloc += ROOMDSC_GROW_RATE;
                        bufmem = (char *) grow(bufmem, rdalloc + 2,
                                               "Extending RDesc Line");
                        }
                    base = bufmem + mem_off;
                    /* Read some more text */
