...
static inline int
chkline(char *p)                /* Test for incomplete rank line */
    {
    if (*p)
	return 0;
...
static inline void
badrank(const char *s)          /* Complain about a bad rank line */
    {
...
rank_line(char *s)
    ...
    *rank.male = *rank.female = 0;
    ...
    if (!rank_item("score", s, rank.score)
        || !rank_item("strength", s, rank.strength)
        || !rank_item("stamina", s, rank.stamina)
        || !rank_item("dexterity", s, rank.dext)
        || !rank_item("wisdom", s, rank.wisdom)
        || !rank_item("experience", s, rank.experience)
        || !rank_item("magic points", s,
                      rank.magicpts)
        || !rank_item("max weight carried", s,
                      rank.maxweight)
        || !rank_item("max. objects carried", s,
                      rank.numobj)
        || !rank_item("player kill value", s,
                      rank.minpksl)
        || !rank_item("task number", s,
                      rank.tasks))
        return;
...
sys_proc(void)
    ...
    *adname = 0;
    ...
    if (mins == -2)
        {
	warne("No %s entry. Default %s enforced: %s.\n", sysopt[SO_SESH], "session time", "15 minutes");
	mins = 15;
        }
    if (invis == -2)
        {
