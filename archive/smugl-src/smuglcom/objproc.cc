...
static inline void
set_mob(void)                   /* Set mobile character for an object */
    {
    int i;
    vocid_t wordno;
...
state_proc(const char *s)
	...
	if ((flag = isoflag2(Word)) == -1)
	    {
            flag = handle_std_flag(Word, state.std_flags,
                                   (flag_t)STATE_STD_FILTER);
            if (flag < 0)
                {
        ...
        else state.flags = (state.flags | (1 << flag));
        }
write:
...
int
isoparm(void)			/* Is it an object parameter? */
    {
    int i;
    char *p;
    for (i = 0; obparms[i]; i++)
