...
add_container(basic_obj boSelf, basic_obj boContainer)
    ...
    if (ncontainers >= ncontainers_allocd)
        ...
        containers = (CONTAINER *)grow(containers, new_size,
                                       "Allocating container memory");
        }
...
is_inside(basic_obj boItem, basic_obj boContainer)
    {
    container_t con;
    for (con = bobs[boContainer]->conTent; con != -1;
	    con = containers[con].conNext)
        {
...
add_basic_obj(BASIC_OBJ *ptr, char type, flag_t flags)
    ...
    if (nbobs >= nbobs_allocd)
        ...
        bobs = (BASIC_OBJ **)grow(bobs, new_size,
                                  "Allocating basic object index memory");
        }
