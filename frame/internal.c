...
{	register char *p;
	...
	p=s; while(*p!=0) { *p=tolower(*p); p++; }

	if(*s=='r') { getrchar(); return; }
	...
	if(*s=='a')
	{
		switch(*(s+1))
		{
			...
			ans("1m"); txs("ANSI control codes now %sABLED.\n",(me->flags & ufANSI)?"EN":"DIS"); ans("0;37m");
			save_me(); return;
		}
	}

	if(*s=='p') {
		tx("Enter old password  : "); Inp(input,250);
		if(input[0]==0) { tx("Cancelled.\n"); return; }
		if(stricmp(input,me->passwd)!=NULL) { tx("Invalid password.\n"); return; }
		tx("Enter new password  : "); Inp(input,250);
		if(input[0]==0) { tx("Cancelled.\n"); return; }
		if(stricmp(input,me->passwd)==NULL) { tx("Passwords are the same.\n"); return; }
		tx("Confirm new password: "); Inp(block,250);
		if(stricmp(input,block)!=NULL) { tx("Passwords did not match.\n"); return; }
