...
{	register char *s,*d,*p,c;	/* Source & Destination */
	...
	s=input; more=10; forced=0; exeunt=0; failed=NO; died=0; donet=-1; ml=0;

	do
	{
		d=block; block[0]=0;
loop:		*d=0; while(isspace(*s)) s++;		/* Skip spaces */
		if(*s==0) { *(s+1)=0; goto proc; }
quotes:		if(block[0]!=0) *(d++)=' ';		/* Force spacing */
		if(*s=='\'' || *s=='\"')
		{
			c=*(d++)=*(s++);		/* Store which */
			...
			if(*s==0) *(s+1)=0;
			*(d++)=*(s++); *d=0;
			if(*s!=0) s++;		/* Skip " or ' at end */
			goto loop;
		}

		p=d;
		while(*s!=0 && !isspace(*s) && *s!='!' && *s!=';' && *s!=',' && *s!='.' && *s!='\"' && *s!='\'') *(d++)=*(s++);
		*d=0; *(d+1)=0;
		if(stricmp(p,"then")==NULL || stricmp(p,"and")==NULL) { *p=0; goto proc; }
		if(*s=='\'' || *s=='\"') goto quotes;
...
proc:		if(*s!=0 && *s!='\'' && *s!='\"') s++;
		...
		if(more!=10)
		{
			ans("3m"); tx((rktab+me->rank)->prompt); txs("%s\n",block); ans("0;37m");
		}
		if(parser()==-1) return;
...
phrase:	wtype[1]=wtype[2]=wtype[3]=wtype[4]=wtype[5]=WNONE;
	...
	{
		if(visible()==YES) action(acp(SGOVANISH),AOTHERS);
		StopFollow(); LoseFollower(); sys(SGO); moveto(x);
		if(visible()==YES) action(acp(SGOAPPEAR),AOTHERS);
		return;
	}
	iocheck(); if(forced!=0 || exeunt!=0 || died!=0) return;
	p=block;
	if(*p=='\"' || *p=='\'')
	{	register char *p2;
		if(*(p+1)==0) return;
...
loop:		while(*p != *p2 && *p2!=0) p2++;
	...
	if(!(vbptr->flags & VB_TRAVEL))
	{
		...
		{
			vbptr=vbtab+word; if(lang_proc(x,1)==0) return;
		}
		vbptr = vbtab+word;
	}
...
caloop:	for(i=0; i<(vbtab+v)->ents; i++)
	{
		...
		for(donet=0; donet < ml; donet++)
		{
			...
			inc=1; act(tt.action,vtabp->pptr); if(inc==1) d=0;
			if(ml < -1) { d=lang_proc(iverb,e); donet=ml+1; break; }
			if(ml<0 || failed!=NO || forced!=0 || died!=0 || exeunt!=0) break;
		}
		if(failed!=NO || forced!=0 || died!=0 || exeunt!=0) break;
after:		if(donet>ml) break;
	...
	{	register int iv;
		iv=iverb; iverb=v;
		if(ttproc() == 0) return 0; else d=-1;
		iverb=iv;
	}
	if(d==-2 && e==0) sys(ALMOST);
	if(d==-1 && e==0)
	{
		if(!(vbptr->flags & VB_TRAVEL)) sys(CANTGO);
		else sys(CANTDO);
	}
	return -1;
}
