iocheck()
{
	REALiocheck(); addcr=NO;
}

REALiocheck()
{
	register int i; register long t,d,f; register char *pt; int p[4];

loopit:	t=0;
	if((ap=(struct Aport *)GetMsg(repbk))!=NULL)
	{
		t=ap->type;
		FreeMem((char *)ap, (long)sizeof(*amul)); goto loopit;
	}
	if(t==-'R') goto here;
...
here:		me2->helping=me2->helped=me2->following=me2->followed=-1;
		...
		if(MyFlag == ROLE_PLAYER)
		{
			...
			{
				i=fread(block,1,800,ifp); block[i]=0; tx(block);
			} while(i == 800);
			fclose(ifp); ifp=NULL;
			sprintf(spc,"\n%s is resetting ... Saving at %ld.\n\nPlease call back later.\n\n",adname,me->score); tx(spc);
			pressret();
		}
		ap->from=-'O';
...
lp:	if(*s==0) { *p=0; return p-ow; }
	if((*p=*(s++)) == '@' && esc(s,p) != 0) { p+=strlen(p); s+=2; }
	else if(*(p++)=='\n') *(p++)='\r';
	goto lp;
}
...
{	register char c;
	...
	switch(tolower(*p))
	{
		case 'm':
			...
			{
				case 'e':
					strcpy(s,me->name); return 1;
				case '!':
					sprintf(s,"%-21s",me->name); return 1;
				case 'r':
					PutRankInto(s); return 1;
				case 'f':
					if(me2->following==-1) strcpy(s,"no-one");
					else strcpy(s,(usr+me2->following)->name);
					return 1;
				case 'g' :
					sprintf(s,"%ld",me2->magicpts); return 1;
				default: return 0;
			}
		case 'g':
			switch(c)
			{
				case 'n':
					strcpy(s,(me->sex==0)?"male":"female"); return 1;
				case 'e':
					strcpy(s,(me->sex==0)?"he":"she"); return 1;
				case 'o':
					strcpy(s,(me->sex==0)?"his":"her"); return 1;
				case 'h':
					strcpy(s,(me->sex==0)?"him":"her"); return 1;
				case 'p':
					sprintf(s,"%ld",me->plays); return 1;
			}
		case 's':
			if(c == 'c') { sprintf(s,"%ld",me->score); return 1; }
			if(c == 'g') { sprintf(s,"%ld",me2->sctg); return 1; }
			if(c == 'r') { sprintf(s,"%ld",me2->strength); return 1;}
			if(c == 't') { sprintf(s,"%ld",me2->stamina); return 1;}
			return 0;
		case 'v':
		...
		case 'p':
			...
			if(isdigit(c)) { fwait(c-'0'); return 1; }
		case 'r':
			if(c == 'e')
			{
				timeto(s,*rescnt);
...
interact(int msg,int n,int d)
{
	...
	IAt=msg; IAd=d; (lstat+n)->IOlock=-1; PutMsg((lstat+n)->rep,(struct Message *)intam);
}
