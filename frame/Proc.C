...
ttproc()
{	register int i,match,dun,l; register struct _TT_ENT *tp;

	exeunt=died=donet=skip=0; failed=NO;
	roomtab=rmtab+me2->room; l=-1; tt.verb=iverb;
	if(roomtab->tabptr==-1) { sys(CANTGO); return 0; }
	dun=-1; ml=roomtab->ttlines; tp=ttp+roomtab->tabptr;

	iocheck(); if(forced!=0 || died!=0 || exeunt!=0) return 0;
more:	i=donet;
	for(i=donet; i<ml; i++)
	{
	...
	if(donet<ml && exeunt==0 && died==0) goto more;
	return 0;
}

act(long ac,long *pt)
{	register long x1,x2; register int i;
	tt.action=ac; tt.pptr=pt;
	if(tt.condition<0) tt.condition=-1-tt.condition;
	...
	if(ac<0)
	{
		...
		switch(ac)
		{
			...
			case ATRAVEL:		x1=donet; x2=ml; if(ttproc()==0) donet=ml=x2; else { ml=x2; donet=x1; }; break;
			case AQUIT:		aquit();
			...
			case AINVENT:		strcpy(block,"You are "); invent(Af); break;
			...
			case ACONTENTS:		str[0]=0; showin(TP1,YES);	break;
			...
			case AFLEE:		dropall((lstat+me2->fighting)->room); clearfight(); break;
			case ALOG:		log(AP1);	break;
			case ACOMBAT:		acombat();	break;
			case AWIELD:		me2->wield = TP1; break;
/* - */			case AFOLLOW:		(lstat+TP1)->followed = me2->unum; me2->following = TP1; break;
...
/* - */			case ASTOPFOLLOW:	StopFollow(); break;
			...
			case ASYNTAX:		asyntax(*(tt.pptr+ncop[tt.condition]),*(tt.pptr+ncop[tt.condition]+1)); break;
			case ASETPRE:		iocopy((lstat+TP1)->pre, AP2, 79); break;
			case ASETPOST:		iocopy((lstat+TP1)->post, AP2, 79); break;
			case ASETARR:		qcopy((lstat+TP1)->arr, AP2, 79); strcat((lstat+TP1)->arr,"\n"); break;
			case ASETDEP:		qcopy((lstat+TP1)->dep, AP2, 79); strcat((lstat+TP1)->dep,"\n"); break;
			...
			case AINC:		if((obtab+CP1)->state != ((obtab+CP1)->nstates-1)) asetstat(CP1, (obtab+CP1)->state+1); break;
			case ADEC:		if((obtab+CP1)->state != 0) asetstat(CP1, (obtab+CP1)->state-1); break;
	...
	{	register int flag;
		...
		if(visible()==YES) { Permit(); action(me2->dep,AOTHERS); Forbid(); }
		ldir=iverb; flag=NO;
		lroom=me2->room; i=me2->light; me2->light=0; Permit(); lighting(Af,AOTHERS);
		...
		if(me2->followed>-1 && me2->followed != Af && (! IamINVIS) && (! IamSINVIS))
		{
			...
			if( ((vbtab+overb)->flags & VB_TRAVEL) || (lstat+me2->followed)->room != lroom || ((lstat+me2->followed)->flags & PFMOVING)) LoseFollower();
			else { DoThis( me2->followed, (vbtab+overb)->id, 1 ); flag=YES; }
		}
		else me2->followed = -1;
		look((rmtab+me2->room)->id, me->rdmode); if(exeunt!=0 || died!=0) return;
 		if(autoexits != 0) exits();
	}
...
{	register int mult,ret,i;
	...
	mult=1; ret=1; tt.condition=(n<0)?-1-n:n;
	if(n<0) { n=-1-n; mult=-1; }
	...
	switch(n)
	{
		...
		case CLIGHT:
			if(lit(me2->room)==NO) ret=-1; break;
		case CISHERE:
			if(isin(CP1,me2->room)==NO) ret=-1; break;
		case CMYRANK:
			if(numb(me->rank+1,CP1) == NO) ret=-1; break;
		case CSTATE:
			if((obtab+CP1)->flags & OF_ZONKED) ret=-1;
		...
		case CRAND:
			if(numb(mod(rnd(),CP1),*(tt.pptr+1)) == NO) ret=-1; break;
		case CRDMODE:
			if(me->rdmode != CP1) ret=-1; break;
		case CONLYUSER:
			for(i=0; i<MAXU; i++)
			if((usr+i)->name[0]!=0 && (lstat+i)->state>1) ret=-1; break;
		case CALONE:
			for(i=0; i<MAXU; i++)
			if(((lstat+i)->room == me2->room) && i != Af) ret=-1; break;
		case CINROOM:
			if(me2->room != CP1) ret=-1; break;
		case COPENS:
			if(!((obtab+CP1)->flags&OF_OPENS)) ret=-1; break;
		case CGOTNOWT:
			if(me2->numobj != 0) ret=-1; break;
		case CCARRYING:
		...
		case CSAMEROOM:
			if((lstat+CP1)->room != me2->room) ret=-1; break;
		case CTOPRANK:
			if(me->rank != ranks-1) ret=-1; break;
		case CSOMEONEHAS:
		...
		case CEMPTY:
			if((obtab+CP1)->inside!=0) ret=-1; break;
		case COBJSIN:
		...
		case CNOUN2:
			...
			break;
		case CAUTOEXITS:
			if(autoexits == 0) return -1; break;
		case CDEBUG:
			if(debug == 0) return -1; break;
		case CFULL:
			if(stfull(CP1,CP2) == NO) return -1; break;
		case CTIME:
			if(numb(*rescnt,CP1) == NO) return -1; break;
		case CDEC:
			if((obtab+CP1)->state == 0) return -1;
			asetstat(CP1, (obtab+CP1)->state-1); break;
		case CINC:
			if((obtab+CP1)->state >= ((obtab+CP1)->nstates-1)) return -1;
			asetstat(CP1, (obtab+CP1)->state+1); break;
		case CLIT:
			if(! (State(CP1)->flags & SF_LIT) ) return -1; break;
		case CFIRE:
			if(! ((obtab+CP1)->flags & OF_SHOWFIRE) ) return -1; break;
		case CHEALTH:
			if(numb((((lstat+CP1)->stamina*100)/(rktab+(usr+CP1)->rank)->stamina),CP2) == NO) ret=-1; break;
		case CMAGIC:
			if(magic(CP1,CP2,CP3)==NO) return -1; break;
		case CSPELL:
			if(numb((lstat+CP1)->wisdom,mod(rnd(),CP2))==NO) ret=-1; break;
		case CIN:
...
strip:	p=*s; while(isspace(*p)) p++; *s=p;
	...
	if(tolower(*p)=='a')
	{
		if(match("an",p)==NULL)   { *s+=2; goto strip; }
		if(match("at",p)==NULL)   { *s+=2; goto strip; }
		if(match("a",p)==NULL)    { *s++;  goto strip; }
	}
	if(tolower(*p)=='t')
	{
		if(match("the",p)==NULL)  { *s+=3; goto strip; }
		if(match("to",p)==NULL)   { *s+=2; goto strip; }
	}
	if(tolower(*p)=='f')
	{
		if(match("for",p)==NULL)  { *s+=3; goto strip; }
		if(match("from",p)==NULL) { *s+=4; goto strip; }
	}
	if(match("is",p)==NULL)   { *s+=2; goto strip; }
	if(match("using",p)==NULL){ *s+=5; goto strip; }
	if(match("with",p)==NULL) { *s+=4; goto strip; }
...
{
	if(n & RAND0) return (int) mod(rnd(),n^RAND0);
	if(n & RAND1) return (int) ( mod(rnd(),n^RAND1) + (n^RAND1)/2 );
	if(n & PRANK) return (int) pRANK(actual(n&-(1+PRANK)));
...
deduct(int plyr, int howmuch)
{
	...
	else sendex(plyr,ADEDUCT,plyr,howmuch,0,0); /* Tell them to clear up! */
}
