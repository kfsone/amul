...
getid()
{	register int i,ok,nrs; register FILE *fp;

	iverb=iadj1=inoun1=iprep=iadj2=inoun2=actor=-1; *(me2->pre)=*(me2->post)=0;
	strcpy(me2->arr,acp(ARRIVED)); strcpy(me2->dep,acp(LEFT));
	last_him=last_her=-1; me->rchar=0;
	...
	do
	{
		...
		strcpy(him.name,block); sprintf(block,"%s%s",dir,plyrfn);
...
loop:	for(i=0;i<rooms;i++,roomtab++)
	...
	if(nrs==0) me2->room=0;
	if(nrs==1) me2->room=start_rm[0];
	if(nrs>1) me2->room=start_rm[mod(rnd(),nrs-1)];
	...
	ans("1m"); if(me->flags&ufANSI) sys(ANSION); ans("0;37m");
...
{	char *p; int i;
	...
	do
	{
		...
		crsys(WHATNAME);
		block[0]=0; Inp(block,NAMEL+1); txc('\n'); if(block[0]==0) quit();
...
{	register int i;
	...
	block[0]=0; Inp(block,3); if(toupper(block[0])!='Y') return -1;

	me->score=0; me->plays=1; me->strength=rktab->strength;
	...
	me->llen=DLLEN; me->slen=DSLEN;

	do
	{
		crsys(WHATGENDER);
		block[0]=0; strcpy(me->name,him.name); Inp(block,2);
		block[0]=toupper(block[0]);
		if(block[0]!='M' && block[0]!='F') { crsys(GENDINVALID); }
	}while(block[0]!='M' && block[0]!='F');
	me->sex=(block[0]=='M') ? 0 : 1;

	do
	{
		crsys(ENTERPASSWD); block[0]=0; Inp(block,10);
		if(strlen(block)<3 || strlen(block)>8) { crsys(PASLENWRONG); }
	...
	crsys(ASK4ANSI); block[0]=0; Inp(block,4);
...
{	int i;
	...
	for(i=0;i<4;i++)
	{
		...
		block[0]=0; Inp(block,10);
		if(stricmp(block,me->passwd)==NULL) break;
	}
	me->plays++;
	if(me->tries>0)
	{
