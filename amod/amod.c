...
void CXBRK()
{
	if(die!=0) return 0;
	die=1;
	if(serop==1) CloseDevice((struct IORequest *)serio);
	if(serio!=NULL) DeleteExtIO((struct IORequest *)serio);
	if(port!=0L) DeletePort(port);
	scrend();
}
...
{	register char *p, ok, arg; register unsigned long bd,was;

	MyTask=FindTask(0); die=0; serop=0; ser_baud=2400;
	...
	arg=0;
	if(argc>1 && *argv[1]=='-') { ser_baud=atoi(argv[1]+1); arg=1; argc--; }
	if(argc>1) strcpy(ser_dev,argv[1+arg]); else strcpy(ser_dev,SERIALNAME);
	if(argc>2) ser_unit=atoi(argv[2+arg]); else ser_unit=0;
	if(argc>3 && toupper(argv[3+arg])=='N') rts=0; else rts=SERF_7WIRE;
	if(argc>4) strcpy(script,argv[4+arg]); else strcpy(script,"loadbbs");

	if((fp=fopen(script,"rb"))==NULL)
		exit(0*printf("Cannot open script %s.\n",script));
	fclose(fp);

	if(!(port=CreatePort(0L,0L))) { printf("Unable to create serial IO port.\n"); exit(0); }
	...
	{	char c;
		...
		{
		loop:	fgets(input,80,fp); if(!feof(fp)) { send(input); send("\r"); Delay(40); goto loop; }
			Delay(32);
			fclose(fp);
		}
		sercmd(CMD_CLEAR);
...
retloop:	sprintf(wintitle,"* AMOD: [%s/%d] %s...",ser_dev,ser_unit,"Waiting for caller");
		SetWindowTitles(win,wintitle,NULL);
		p=input; *p=0;

		while((*p=(char)sgetc())!=13 && *p!=10) p++;
		*p=0;

		if(p==input) goto retloop;	/* Ignore blank strings */

		fp=fopen("RAM:ResultCodes.TXT","rb");
		while(!feof(fp))
		{
			ok=0; fscanf(fp,"%s",match); bd=atoi(match);
			fgets(match,80,fp); if(feof(fp)) continue;
			while(isspace(match[0])) strcpy(match,match+1);
			match[strlen(match)-1]=0;
			if(match[0]=='-') { ok=0; strcpy(match,match+1); }
			else ok=1;
			if(stricmp(match,input)==NULL) break;
		};

		fclose(fp); sercmd(SDCMD_QUERY);
		if(bd<300 && ok!=0) { CloseDevice((struct IORequest *)serio); opendev(); continue; }
		if(ok==0) goto retloop;
...
menu:		sercmd(SDCMD_QUERY); if( NOCARRIER ) continue; modemcfg();
		send("\r\nCodeOMatic System Menu:\r\n\r\n");
		send("Select:\r\n\r\n  A  -  Enter the Multi-user Dungeon...\r\n");
		send("  B  -  Enter the CodeOMatic BBS...\r\n");
		send("  G  -  Goodbye and hang up\r\n\r\n"); sercmd(CMD_CLEAR);
		send("Which (A,B or G): "); sercmd(SDCMD_QUERY); Delay(8);
		input[0]=(char)sgetc(); sercmd(SDCMD_QUERY);
		sprintf(match,"%c\r\n\r\n",c=toupper(input[0])); send(match);
		if(c!='A' && c!='B' && c!='G')
		{
			send("\r\nInvalid command!\r\n"); sercmd(SDCMD_QUERY);
			if( NOCARRIER )
			{
				wsend("Lost carrier!\n"); sercmd(CMD_CLEAR);
				CloseDevice((struct IORequest *)serio); opendev();
				sercmd(CMD_CLEAR); continue;
			}
			else goto menu;
		}
		if(c=='A')
...
goodbye:	if(c=='G')
		{
			send("Thankyou very much for calling the CodeOMatic BBS...\r\n\r\nCiao!\r\n\r\n<Click>\r\n");
			Delay(20);
			continue;
		}
		if(c=='B')
		{
			...
			if(strcmp(ser_dev,"serial.device")!=NULL) sprintf(input,"paragon 2 h b%ld spawned",(ser_baud>2400)?ser_baud:bd);
			else sprintf(input,"paragon 1 h b%ld l9600 spawned",(ser_baud>2400)?ser_baud:bd);
			sprintf(wintitle,"* AMod: [%s/%d] %s...",ser_dev,ser_unit,"Entered the BBS...");
		}
		sercmd(CMD_CLEAR); sercmd(SDCMD_QUERY);
		if( NOCARRIER ) continue;
		SetWindowTitles(win,wintitle,NULL);
		sprintf(block,"Executing '%s'\n",input); wsend(block); Execute(input,0,0);
		Delay(32); if(c=='B') { c='G'; goto goodbye; }
		sercmd(CMD_CLEAR); modemcfg(); sercmd(SDCMD_QUERY);
		if( NOCARRIER ) continue;
		sercmd(CMD_CLEAR); goto menu;
	} while(p==p);
}

sercmd(int cmd)
{
	serio->IOSer.io_Command=cmd; DoIO((struct IORequest *)serio);
	Delay(10);
}
...
loop:	serio->IOSer.io_Command=CMD_READ; SendIO((struct IORequest *)serio);
	Wait(-1);
	if(GetMsg((struct MsgPort *)port) != NULL) return (int) c&255;
...
ssend(char *s)
{
	serio->IOSer.io_Data=(APTR)s; serio->IOSer.io_Length=strlen(s);
	serio->IOSer.io_Command=CMD_WRITE; DoIO((struct IORequest *)serio);
}
...
modeminit()
{
	serio->io_ExtFlags=0; serio->io_Baud=ser_baud; serio->io_SerFlags = SERF_SHARED + rts;
	serio->io_Status=136; serio->IOSer.io_Message.mn_ReplyPort=port;
}

modemcfg()
{
	modeminit(); sercmd(SDCMD_SETPARAMS); Delay(20);
}
...
opendev()
{
	serop=0;
	modeminit();
	if( OpenDevice(ser_dev,ser_unit,(struct IORequest *)serio,0) != 0) CXBRK(printf("Cannot access device \"%s\"!\n",ser_dev));
	serio->IOSer.io_Device=serio->IOSer.io_Device;
	serio->IOSer.io_Unit=serio->IOSer.io_Unit;

	sercmd(CMD_RESET); sercmd(CMD_CLEAR); sercmd(CMD_FLUSH); sercmd(SDCMD_QUERY);

	modemcfg();
	if(serio->IOSer.io_Error!=NULL) CXBRK(printf("Unable to configure %s\n",ser_dev));
	serop=1;
}
...
{
	AbortIO((struct IORequest *)serio);
loop:	CloseDevice((struct IORequest *)serio); Delay(40); opendev();
	send("\r\n"); Delay(30);
	CloseDevice((struct IORequest *)serio); Delay(40); opendev();
	Delay(20); send("+++"); Delay(60); send("ATZ\r\n"); Delay(100);
	send("\r\n"); Delay(20);
	sercmd(CMD_FLUSH); Delay(10); sercmd(CMD_CLEAR); Delay(10);
	sercmd(SDCMD_QUERY); /* Check that we lost 'im! */
	if( !NOCARRIER ) { Delay(32); goto loop; }
}
...
OpenMyWindow()
{
	...
	if(ITBase!=NULL) win=FlashyOpenWindow(&NewWindowStructure1);
	else win=OpenWindow(&NewWindowStructure1);
	...
	{
		sprintf(block,"Unable to open %s!","console.device"); puts(block);
		scrend();
	}
...
scrend()
{
	...
	if(win!=NULL)
	{
		if(ITBase!=NULL) FlashyCloseWindow(win);
		else CloseWindow(win);
	}
	if(IntuitionBase != 0L) CloseLibrary((struct Library *)IntuitionBase);
	if(ITBase != 0L) CloseLibrary((struct Library *)ITBase);
	exit(0);
}
